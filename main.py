...
class ImageBase64Converter:
    ...
    def create_widgets(self):
        ...
        self.preview_button = ctk.CTkButton(
            self.button_frame, text="👀 预览Base64图片", command=self.preview_image, font=("Arial", 12), width=120, corner_radius=10,
            fg_color="#2196F3", hover_color="#1e88e5"
        )
        self.preview_button.pack(side="left", padx=10, pady=10)

        self.download_button = ctk.CTkButton(
            self.button_frame, text="💾 保存为图片", command=self.download_image, font=("Arial", 12), width=120, corner_radius=10,
        ...
        self.encrypt_button = ctk.CTkButton(
            self.button_frame, text="🔒 加密", command=self.encrypt_image, font=("Arial", 12), width=120, corner_radius=10,
            fg_color="#4CAF50", hover_color="#45a049"
        )
    ...
    def encrypt_image(self):
        ...
        if self.file_path:
            # 加密并保存加密后的图片
            self.convert_image_to_base64(self.file_path)
            # messagebox.showinfo("提示", "图片加密成功！")
        else:
    ...
    def browse_image(self):
        ...
        if file_path:
            ...
            return
        file_size = os.path.getsize(self.file_path) / (1024 * 1024)  # 文件大小（MB）
        if file_size > 10:  # 限制图片大小为 10MB
            messagebox.showwarning("警告", "图片文件过大，可能发生卡顿。")
        self.image_path_entry.delete(0, "end")
        self.image_path_entry.insert(0, self.file_path)
